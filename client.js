...
async function getCloudOrders() {
    ...
    try {
        ...
        const data = await response.json();
        debugLog('Orders fetched:', data.record?.orders?.length || 0);
...
async function saveCloudOrders(orders) {
    ...
    try {
        ...
        const response = await fetch(`${CONFIG.API_URL}/b/${CONFIG.BIN_ID}`, {
            method: 'PUT',
            headers: headers,
            body: JSON.stringify(orders)  // Send just the orders array
        });
...
async function submitOrder() {
    ...
    if (!CONFIG.JSONBIN_API_KEY || CONFIG.JSONBIN_API_KEY.includes('YOUR_ACTUAL')) {
        showError('Please configure your JSONBin.io API key');
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (!CONFIG.JSONBIN_API_KEY || CONFIG.JSONBIN_API_KEY.includes('YOUR_ACTUAL')) {
        ...
        const testResult = document.getElementById('testResult');
        if (testResult) {
            testResult.innerHTML = '⚠️ Configure API key first';
    ...
    console.log('🔑 API Key:', CONFIG.JSONBIN_API_KEY ? '✓ Present' : '✗ Missing');
    console.log('📦 Bin ID:', CONFIG.BIN_ID ? '✓ Present' : '✗ Missing');
});
...
window.calculateTotal = calculateTotal;// client.js - PureFlow Online Client
// ================== CONFIGURATION ==================
const CONFIG = {
